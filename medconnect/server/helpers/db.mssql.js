...
export async function getPool() {
  ...
  const config = {
    ...
    options: {
      encrypt:
        (process.env.MSSQL_ENCRYPT ?? process.env.DB_ENCRYPT ?? "")
          .toString()
          .toLowerCase() === "true",
      trustServerCertificate:
        (process.env.MSSQL_TRUST_SERVER_CERT ?? process.env.DB_TRUST_CERT ?? "")
          .toString()
          .toLowerCase() !== "false",
...
export async function findUserByEmail(email) {
  ...
  const result = await p
    .request()
    .input("email", sql.NVarChar, email.toLowerCase()).query(`
...
export async function findUserByPhone(rawPhone) {
  ...
  const p = await getPool();
  const result = await p.request().input("phone", sql.NVarChar, phone).query(`
