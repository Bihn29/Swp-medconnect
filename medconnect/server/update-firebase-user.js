...
try {
  ...
  if (user) {
    console.log('✅ Updated user with FirebaseUID:', user.fullName, '(', user.email, ')');
    ...
    if (timeSlots.length === 0) {
      console.log('❌ No time slots found, copying from existing doctor...');
      
      // Get existing doctor's slots to copy
      const existingDoctor = await Doctor.findOne({ _id: { $ne: doctor._id } }).lean();
      if (existingDoctor) {
  ...
  // Final verification
  const finalUser = await User.findOne({ firebaseUID }).lean();
  ...
  if (finalSlots.length > 0) {
    console.log('\n📅 Sample time slots:');
    finalSlots.slice(0, 5).forEach((slot, index) => {
  ...
  console.log('👤 Doctor: Bùi Ngọc Lan');
