          ...
          <div className="flex bg-white border border-gray-200 rounded-lg overflow-hidden">
            <button
              className={`px-6 py-2 font-medium transition-colors ${
                viewMode === "month" ? "bg-primary text-white" : "text-gray-600 hover:bg-gray-50"
              }`}
            ...
            <button
              className={`px-6 py-2 font-medium transition-colors ${
                viewMode === "week" ? "bg-primary text-white" : "text-gray-600 hover:bg-gray-50"
              }`}
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-7 gap-px bg-gray-200 border border-gray-200 rounded-lg overflow-hidden">
            {days.map((dayObj, index) => {
              const events = dayObj.isCurrentMonth ? getEventsForDay(dayObj.day) : []
              return (
                <div key={index} className={`min-h-[100px] p-2 ${dayObj.isCurrentMonth ? "bg-white" : "bg-gray-50"}`}>
                  <div className={`font-semibold mb-1 ${dayObj.isCurrentMonth ? "text-gray-900" : "text-gray-400"}`}>
                    {dayObj.day}
                  </div>
                    ...
                    {events.map((event) => (
                      <div
                        ...
                        className={`px-2 py-1 rounded text-xs font-medium cursor-pointer hover:scale-105 transition-transform ${
                          event.type === "online" ? "bg-primary/15 text-primary" : "bg-orange-100 text-orange-700"
                        }`}
