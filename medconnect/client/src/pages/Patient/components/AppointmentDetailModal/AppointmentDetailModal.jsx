...
import { Modal, Card, Typography, Button, Space, Tag, Divider, Spin, message } from 'antd';
import { 
  CalendarOutlined, 
  ClockCircleOutlined, 
  EnvironmentOutlined, 
...
const AppointmentDetailModal = ({ 
  ...
  const handleVideoCall = () => {
    if (appointment?.status === 'accepted' && appointment?.mode === 'online') {
      window.open(`/benh-nhan/video-call/${appointment._id}`, '_blank');
    }
  };
    ...
    <Modal
      ...
      footer={[
        ...
        appointment?.status === 'accepted' && appointment?.mode === 'online' && (
          <Button key="video" type="primary" onClick={handleVideoCall}>
            <VideoCameraOutlined /> Tham gia video call
          </Button>
        )
      ]}
      width={600}
        ...
        <Card size="small" title="Thông tin bác sĩ">
          <Space direction="vertical" size="small">
            <div>
              ...
              <Text strong>{appointment.doctorId?.fullName || 'Chưa xác định'}</Text>
            </div>
            <div>
              <Text type="secondary">
                Chuyên khoa: {appointment.doctorId?.specializationIds?.[0]?.name || 'Chưa xác định'}
              </Text>
            </div>
        ...
        <Card size="small" title="Thông tin khám">
          <Space direction="vertical" size="small">
            <div>
              ...
              <Text strong>
                {appointment.mode === 'online' ? 'Khám online' : 'Khám tại phòng khám'}
              </Text>
            </div>
