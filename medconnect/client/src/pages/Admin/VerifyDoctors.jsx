...
const VerifyDoctors = () => {
  ...
  const filterDoctors = (doctors, searchTerm) => {
    ...
    return doctors.filter(doctor => {
      ...
      const specialty = (doctor.specialty || doctor.specializationName || '').toLowerCase();
  ...
  const tabItems = [
        ...
        <div className="doctors-list">
          {filteredPendingDoctors.length === 0 ? (
            <div className="empty-state">
              {searchTerm ? 'Không tìm thấy bác sĩ nào' : 'Không có bác sĩ nào chờ xác minh'}
            </div>
          ) : (
            filteredPendingDoctors.map(doctor => renderDoctorRow(doctor, 'pending'))
          )}
        </div>
      ),
    },
      ...
      children: (
        <div className="doctors-list">
          {filteredVerifiedDoctors.length === 0 ? (
            <div className="empty-state">
              {searchTerm ? 'Không tìm thấy bác sĩ nào' : 'Không có bác sĩ nào đã xác minh'}
            </div>
          ) : (
            filteredVerifiedDoctors.map(doctor => renderDoctorRow(doctor, 'verified'))
          )}
        </div>
      ),
    },
    {
      ...
      label: (
        <span className="tab-label">
          <ExclamationCircleOutlined /> Đã từ chối ({filteredRejectedDoctors.length})
        </span>
      ),
      children: (
        <div className="doctors-list">
          {filteredRejectedDoctors.length === 0 ? (
            <div className="empty-state">
              {searchTerm ? 'Không tìm thấy bác sĩ nào' : 'Không có bác sĩ nào bị từ chối'}
            </div>
          ) : (
            filteredRejectedDoctors.map(doctor => renderDoctorRow(doctor, 'rejected'))
          )}
        </div>
      ),
    },
  ...
  const renderDetailModal = () => {
    ...
    const doctorPhone = doctor.phone || doctor.userId?.phone || 'Chưa có số điện thoại';
    
    // Get specialty from database
    let specialty = doctor.specialty || 'Chưa có chuyên khoa';
    if (!doctor.specialty && doctor.specializationIds && Array.isArray(doctor.specializationIds) && doctor.specializationIds.length > 0) {
    ...
    const clinicName = doctor.clinicName || doctor.hospital || doctor.clinicDefaultId?.name || 'Chưa có thông tin';
    
    // Get experience from database - extract number if it's a formatted string
    let experience = 0;
    if (doctor.yearsExperience !== undefined && doctor.yearsExperience !== null) {
    ...
    if (doctor._statusFromTab) {
      ...
      doctorStatus = doctor._statusFromTab === 'verified' ? 'approved' : 
                     doctor._statusFromTab === 'rejected' ? 'rejected' : 'pending';
    ...
    if (doctor.documents && Array.isArray(doctor.documents) && doctor.documents.length > 0) {
              ...
              <div className="detail-header-info">
                <div className="detail-name-row">
                  ...
                  <span className={`status-badge ${doctorStatus}-badge`}>
                    ...
                    {doctorStatus === 'pending' ? 'CHỜ XÁC MINH' : doctorStatus === 'approved' ? 'Đã xác minh' : 'Đã từ chối'}
                  </span>
                </div>
