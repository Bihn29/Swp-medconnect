...
export default function InvoiceManagement() {
  ...
  const generateInvoiceDocHTML = (invoice) => {
    ...
    const paidDate = invoice.paidAt ? formatDateForDoc(invoice.paidAt) : "Chưa thanh toán";
    const patientDob = invoice.patientDateOfBirth ? formatDateOnlyForDoc(invoice.patientDateOfBirth) : "N/A";
    const genderText = invoice.patientGender === "male" ? "Nam" : invoice.patientGender === "female" ? "Nữ" : invoice.patientGender || "N/A";
    ...
    const itemsRows = invoice.items?.map(item => `
      <tr>
        ...
        <td style="border:1px solid #ddd; padding:8px; text-align:right">${formatCurrency(item.unitPrice || 0)}</td>
        <td style="border:1px solid #ddd; padding:8px; text-align:right">${formatCurrency(item.lineTotal || 0)}</td>
      </tr>
    `).join("") || "";
        ...
        <div class="section">
          ...
          <div class="kv-2col">
            ...
            <div class="kv-item">
              ...
              <div class="kv-value">${invoice.invoiceType === "booking" ? "Thanh toán đặt lịch" : "Thanh toán dịch vụ"}</div>
            </div>
            <div class="kv-item">
  ...
  const handleDeleteInvoice = async (invoice) => {
    ...
    try {
      const response = await api.delete(`/api/managers/invoices/${invoice._id}`);
      ...
      const errorMessage = error.response?.data?.message || "Có lỗi khi xóa hóa đơn. Vui lòng thử lại.";
      alert(errorMessage);
    }
  };
